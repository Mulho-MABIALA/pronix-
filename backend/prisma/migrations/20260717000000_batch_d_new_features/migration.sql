...
-- Convertie depuis migrations/batch_d_new_features.sql (appliquée manuellement
-- via psql sur le serveur) en vraie migration Prisma, pour que
-- `npx prisma migrate deploy` puisse reconstruire ce schéma sur une base neuve.
-- Toutes les instructions sont idempotentes (IF NOT EXISTS) : aucun risque si
-- rejouée sur une base où elle a déjà été appliquée à la main.
-- =============================================================================
...
-- NOTE : la fonctionnalité "combinés + portefeuille virtuel" a été retirée du
-- produit ensuite (voir tâche #160) — ces tables restent en base pour
-- compatibilité historique mais ne sont plus utilisées par le code actuel.
