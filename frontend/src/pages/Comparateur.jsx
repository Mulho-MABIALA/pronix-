...
function FormBadge({ r }) {
  const color = r === 'W' ? 'bg-primary-500 text-white' : r === 'D' ? 'bg-surface-500 text-gray-200' : 'bg-red-500/80 text-white';
...
function TeamPicker({ label, team, onPick, onClear }) {
  ...
  if (team) {
    return (
      <div className="flex items-center gap-2.5 p-2.5 rounded-2xl border border-white/[0.08]" style={{ background: 'rgba(255,255,255,0.03)' }}>
        <TeamLogo logo={team.logo} teamId={team.id} name={team.name} size={28} />
        <p className="flex-1 text-sm font-semibold text-gray-100 truncate">{team.name}</p>
        <button onClick={onClear} className="text-gray-400 hover:text-gray-200 transition-colors shrink-0" aria-label="Retirer">
          <X size={15} />
        </button>
      </div>
    );
  ...
  return (
    <div className="relative">
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl border border-white/[0.08]" style={{ background: 'rgba(255,255,255,0.03)' }}>
        <Search size={15} className="text-gray-400 shrink-0" />
        <input
          value={query}
          onChange={handleChange}
          placeholder={label}
          className="flex-1 min-w-0 bg-transparent text-sm text-gray-100 placeholder-gray-500 outline-none"
          autoComplete="off"
          spellCheck={false}
        />
      </div>
      {query.length >= 2 && (
        <div className="absolute z-10 mt-1 w-full rounded-xl border border-white/[0.1] overflow-hidden max-h-64 overflow-y-auto"
          style={{ background: 'rgba(18,19,22,0.99)' }}>
          {loading && <div className="px-3 py-3 text-xs text-gray-400">…</div>}
          ...
          {!loading && results.length === 0 && (
            <div className="px-3 py-3 text-xs text-gray-400">{noResultsLabel}</div>
          )}
        </div>
      )}
    </div>
...
export default function Comparateur() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-6 animate-fade-in">
      ...
      <div>
        <h1 className="font-display font-bold text-xl text-gray-100 mb-1">{t('comparator.title')}</h1>
        <p className="text-sm text-gray-300">{t('comparator.subtitle')}</p>
      </div>
      ...
      {stats1 && stats2 && (
        <>
          <section className="bento-card p-4 space-y-3">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('comparator.formTitle')}</h2>
            ...
            <div className="flex items-center justify-between gap-2 pb-1 border-b border-white/[0.06]">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-xs font-semibold text-gray-200 truncate">{team1.name}</span>
              </div>
              <div className="flex items-center gap-2 min-w-0 flex-row-reverse">
                <TeamLogo logo={team2.logo} teamId={team2.id} name={team2.name} size={18} />
                <span className="text-xs font-semibold text-gray-200 truncate">{team2.name}</span>
              </div>
            </div>

            {ROWS.map(([label, v1, v2]) => (
              <div key={label} className="flex items-center justify-between text-sm">
                <span className={`font-bold tabular-nums w-12 text-left ${v1 > v2 ? 'text-primary-400' : 'text-gray-200'}`}>{v1}</span>
                <span className="text-xs text-gray-300 flex-1 text-center truncate px-2">{label}</span>
                <span className={`font-bold tabular-nums w-12 text-right ${v2 > v1 ? 'text-primary-400' : 'text-gray-200'}`}>{v2}</span>
              </div>
            ))}
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/[0.06]">
              ...
              <span className="text-[11px] text-gray-400 shrink-0 px-2">{t('comparator.recentForm')}</span>
          ...
          <section className="bento-card p-4 space-y-3">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('comparator.h2hTitle')}</h2>
            {!h2h?.matches?.length ? (
              <p className="text-sm text-gray-300">{t('comparator.noH2h')}</p>
            ) : (
              <>
                <div className="flex items-center justify-center gap-6 text-sm py-1">
                  ...
                  <span className="text-xs text-gray-400">{h2h.draws} {t('comparator.draws').toLowerCase()}</span>
                  ...
                  {h2h.matches.map((m) => (
                    <div key={m.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-800 border border-surface-700 text-xs">
                      ...
                      <span className="flex-1 text-gray-200 truncate">
                        {m.homeTeam} <strong className="text-gray-100">{m.homeScore}-{m.awayScore}</strong> {m.awayTeam}
                      </span>
                    </div>
                  ))}
                </div>
      ...
      {(!team1 || !team2) && (
        <div className="bento-card text-center py-10 px-4 text-sm text-gray-300">
          {t('comparator.hint')}
        </div>
      )}
    </div>
