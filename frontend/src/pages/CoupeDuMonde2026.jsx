  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <section className="relative overflow-hidden mx-4 mt-6 mb-6 rounded-3xl border border-overlay/[0.08]"
        ...
        <div className="relative px-6 py-10 md:py-14 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary-500/30 bg-primary-500/10 text-primary-400 text-xs font-semibold mb-5">
            ...
            FIFA World Cup™
            {wcPhase === 'finished' && <span className="text-ink-4">· {t('worldCup.finished')}</span>}
          </div>
          ...
          <p className="text-ink-4 text-sm md:text-base mb-6 max-w-md mx-auto">
            {t('worldCup.subtitle')}<br />
            <span className="text-ink-3 text-xs">{t('worldCup.dates')}</span>
          </p>
          ...
          {liveMatches.length > 0 && (
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-live-500/15 border border-live-500/30 text-live-400 font-semibold text-sm">
              ...
              {liveMatches.length} {liveMatches.length > 1 ? t('worldCup.liveMatches') : t('worldCup.liveMatch')}
            </div>
          )}

          {/* Pays hôtes */}
          <div className="flex items-center justify-center gap-4 mt-6 text-xs text-ink-4">
            <span>🇺🇸 {t('worldCup.usa')}</span>
            <span>·</span>
            <span>🇨🇦 {t('worldCup.canada')}</span>
            <span>·</span>
            <span>🇲🇽 {t('worldCup.mexico')}</span>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 mb-6">
        {[
          { label: t('worldCup.teams'), value: '48', icon: '🏟️' },
          { label: t('worldCup.matches'), value: '104', icon: '⚽' },
          { label: t('worldCup.cities'), value: '16', icon: '📍' },
        ].map((item) => (
      ...
      <section className="px-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/matchs" className="text-xs text-ink-4 hover:text-ink-3 flex items-center gap-1 transition-colors">
            {t('worldCup.allMatches')} <ChevronRight size={12} />
          </Link>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="card p-10 text-center">
            ...
            {wcPhase === 'finished' ? (
              <>
                <p className="text-ink-4 font-medium">{t('worldCup.finishedTitle')}</p>
                <p className="text-ink-4 text-sm mt-2">
                  {t('worldCup.finishedDesc')}
                </p>
              </>
            ) : (
              <>
                <p className="text-ink-4 font-medium">{t('worldCup.upcomingTitle')}</p>
                <p className="text-ink-4 text-sm mt-2">
                  {t('worldCup.upcomingDesc')}
                </p>
              </>
            )}
            <Link to="/matchs"
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
          {WC_GROUPS.map((g) => (
            <div key={g} className="bento-card p-3 text-center cursor-default hover:-translate-y-0.5 transition-transform">
              ...
              <p className="text-xs text-ink-4 mt-0.5">{t('worldCup.groupTeams')}</p>
            </div>
          ))}
        </div>
      </section>
